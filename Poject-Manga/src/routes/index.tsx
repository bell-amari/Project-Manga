    ...
    <div>
      <section className="relative overflow-hidden border-b-2 border-ink">
        ...
        <div className="mx-auto grid max-w-7xl gap-10 px-4 py-16 sm:px-6 md:grid-cols-2 md:py-24 lg:py-32">
          <div className="flex flex-col justify-center">
            ...
            <div className="mt-8 flex flex-wrap gap-3">
              <Link to="/login" className="cursor-pointer">
                <Button size="lg" className="h-12 cursor-pointer border-2 border-ink font-display uppercase tracking-wider shadow-stamp">
                  Start your shelf <ArrowRight className="ml-2 h-5 w-5" />
                </Button>
              </Link>
              <a href="#top-reads" className="cursor-pointer">
                <Button size="lg" variant="outline" className="h-12 cursor-pointer border-2 border-ink font-display uppercase tracking-wider hover:bg-ink hover:text-paper">
                  Browse top reads
                </Button>
              </a>
            </div>
          ...
          <div className="relative">
            <div className="absolute -inset-4 translate-x-3 translate-y-3 border-2 border-ink bg-primary" aria-hidden />
            <img
              src={heroImg}
      ...
      <section id="top-reads" className="border-b-2 border-ink bg-paper">
        <div className="mx-auto max-w-7xl px-4 py-20 sm:px-6">
          ...
          {mangaError && (
            <p className="mt-10 text-sm font-bold text-red-600">{mangaError}</p>
          )}
            ...
            <ol className="mt-10 grid gap-6 sm:grid-cols-2 lg:grid-cols-3">
              {topManga.map((m) => (
                ...
                >
                  <a
                    href={`/manga/${m.id}`}
                    className="block h-full cursor-pointer p-6 focus:outline-none focus-visible:ring-4 focus-visible:ring-primary/40"
                  ...
                  >
                    ...
                    <div className="mt-6 flex aspect-square w-full items-center justify-center overflow-hidden border-2 border-ink bg-paper">
                      <img
        ...
        <div className="mx-auto grid max-w-7xl gap-12 px-4 py-20 sm:px-6 md:grid-cols-2">
          <div>
            ...
            <div className="mt-8 flex gap-3">
              <Link to="/login" className="cursor-pointer">
                <Button size="lg" className="h-12 cursor-pointer border-2 border-paper bg-primary font-display uppercase tracking-wider text-primary-foreground hover:bg-primary/90">
          ...
          <div className="grid items-center gap-12 md:grid-cols-2">
            <div>
              ...
              <ul className="mt-6 space-y-3">
                {["Track owned, reading, finished & wishlist", "Personal 1–5 rating system", "Private notes per volume"].map((f) => (
              ...
              <div className="grid grid-cols-3 gap-2 border-2 border-ink bg-card p-4 shadow-stamp">
                {topManga.slice(0, 9).map((m) => (
                  <a
                    key={m.id}
                    href={`/manga/${m.id}`}
                    className="aspect-[2/3] cursor-pointer overflow-hidden border border-ink bg-paper transition-transform hover:-translate-y-1 focus:outline-none focus-visible:ring-4 focus-visible:ring-primary/40"
                    aria-label={`Open ${m.title.english || m.title.romaji}`}
                  >
                    <img
                      src={m.coverImage.large}
